...
class MB_straightenUVs():
    def __init__(self):
        print("Straighten UV Shells Tool, 1.0.1")
        self.errorMessage = "Points not on the same edge loop or mesh not continuous surface!"
        self.totalNumShells = 0
        self.shellsDone = 0
        if pm.window("MBUVStraightenUI", exists=True):
            pm.deleteUI("MBUVStraightenUI")
        self.gui()

    def gui(self):
        self.win = pm.window("MBUVStraightenUI", title="Straighten UVs", rtf=True, s=True, tlb=False)
        self.layout = pm.columnLayout(adjustableColumn=True)
        pm.text(label="")
        pm.text(label="Straighten UVs", font="boldLabelFont", align="center", backgroundColor=(0.3, 0.3, 0.3), height=30)
        pm.separator(height=10, style='none')
        self.alignCheck = pm.checkBox(v=False, label="Align Shells", ann="Use the edge loops, post-straighten, as a ruler for snapping shell to a 90-degree angle")
        pm.separator(height=10, style='none')
        self.doStraighten = pm.checkBox(v=True, label="Straighten Shells", offCommand=self.StraightenOff, ann="Straighten edge loop or just align it")
        pm.separator(height=10, style='none')
        pm.text(label="Straighten Method", bgc=[0.3, 0.3, 0.3])
        pm.separator(height=5, style='none') 
        self.useUnfold = pm.checkBox(v=True, label="Use Unfold", onCommand=self.unfoldOn, ann="Use the old Unfold to straighten the rest of the shell (default)")
        pm.separator(height=10, style='none') 
        self.useFishbone = pm.checkBox(v=False, label="Use Strict Straighten (SLOW)", onCommand=self.fishboneOn, offCommand=self.fishboneOff, ann="Strict straighten, can give wonky results")
        pm.separator(height=10, style='none')
        self.btn = pm.button(label="Straighten", ann="Usage:\n\nSelect either edge loops or UVs (for border edges) on an edge loop of shells\nYou can select multiple shells\nChoose Align, Straighten or both\nSelect straighten method (Unfold is by far the fastest, Strict can overwhelm the machine!)\n\nClick this button\n\nNote: For Align only, you can select just an edge or any 2 UV points on a shell", w=250, command=self.goThroughEdgeloopsAndStraighten)
        pm.separator(height=10, style='none')
        self.progressor = pm.progressBar(visible=False, enable=False, progress=0, width=250)
        pm.separator(height=10, style='none')
        self.win.show()
